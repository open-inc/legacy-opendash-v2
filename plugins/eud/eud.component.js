...
let standardColors = ['#7cb5ec', '#434348', '#90ed7d', '#f7a35c', '#8085e9', '#f15c80', '#e4d354', '#2b908f', '#f45b5b', '#91e8e1'];

class controller {

    static get $inject() {
        return ['opendash/services/eud', 'opendash/services/notification', 'moment'];
    }
    ...
    init(sort) {
        ...
        $eud.getItems().then(items => {
            if (sort == 0) {
                ...
                this.items = _.orderBy(items, [item => item.name.toLowerCase(), item => item.valueName.toLowerCase()], ['asc']);
            } else if (sort == 2) {
            ...
            for (var i = 0; i < this.history.length; i++) {
                ...
                plotLines.push({
                    ...
                    label: {
                        x: 65,
                        zIndex: 999999,
                        text: 'Durchschnitt ' + this.history[i].item.name + ' (' + mean.toFixed(2) + ')',
                    },
                });
            }
        } else if (number == 1) { //Median
            for (var i = 0; i < this.history.length; i++) {
                let values = [];
                _.each(this.history[i].history, z => {
                    values.push(z.value);
                ...
                plotLines.push({
                    ...
                    label: {
                        x: 65,
                        zIndex: 999999,
                        text: 'Median ' + this.history[i].item.name + ' (' + median.toFixed(2) + ')',
                    },
                });
            }
            ...
            for (var i = 0; i < this.history.length; i++) {
                ...
                plotLines.push({
                    ...
                    label: {
                        x: 65,
                        zIndex: 999999,
                        text: 'Maximal ' + this.history[i].item.name + ' (' + max.toFixed(2) + ')',
                    },
                });
            }
        } else if (number == 3) { //Min
            for (var i = 0; i < this.history.length; i++) {
                let values = [];
                _.each(this.history[i].history, z => {
                    values.push(z.value);
                ...
                plotLines.push({
                    ...
                    label: {
                        x: 65,
                        zIndex: 999999,
                        text: 'Minimal ' + this.history[i].item.name + ' (' + min.toFixed(2) + ')',
                    },
                });
            }
        } else if (number == 4) { //OWN
    ...
    exportData() {
        for (let index in this.history) {
            ...
            let end = moment(this.history[index].history[this.history[index].history.length - 1].date).format('L');
            let dataClean = [];
            let duplicateCheck = [];
            this.history[index].history.forEach(function (element) {
                ...
                if (duplicateCheck.length < 1) {
                    ...
                    if (duplicateCheck[0] === moment(element.date).format('YYYY-MM-DD') && duplicateCheck[1] === moment(element.date).format('HH:mm:ss')) {
                        ignore = true;
                    } else {
                        ignore = false;
            ...
            downloadLink.download = this.history[index].item.name + '(' + start + '-' + end + ').csv';
