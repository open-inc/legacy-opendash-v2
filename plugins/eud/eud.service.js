...
export default class EUDService {
    ...
    static get $inject() {
        return ['opendash/services/data', 'opendash/services/dashboard', 'opendash/services/modal', '$q', 'opendash/services/env', 'opendash/services/header', 'moment'];
    }
    ...
    newWidget() {
        let options = {
            ...
            template: '<od-eud-editor ng-if="loaded" selected="selected" settings="settings" save="save" ></od-eud-editor>',
        };
    ...
    getHistory(items, selected, settings) {
        ...
        return $q.all(_.map(items, item => {
            ...
            return $data.get(item.id).history(history).then(data => {
    ...
    getChart(items, settings) {
        ...
        switch (settings.chart.type) {
            default:
                ...
                _.set(highchartConfig, 'tooltip.pointFormat', '{series.name}: {point.y:.2f}');
                ...
                _.set(highchartConfig, 'chart.animation', (settings.chart.animation) ? true : false);
                _.set(highchartConfig, 'credits.enabled', false);
                _.set(highchartConfig, 'chart.zoomType', (settings.chart.zoom) ? 'x' : false);
                _.set(highchartConfig, 'plotOptions.series.enableMouseTracking', (settings.chart.tooltips) ? true : false);
                _.set(highchartConfig, 'legend.enabled', (settings.chart.legend) ? true : false);
                _.set(highchartConfig, 'plotOptions.series.animation', (settings.chart.animation) ? true : false);
        ...
        try {
            ...
            if (axisCount.length == 2) { //2 Achsen
                _.set(highchartConfig, 'yAxis[0].title.text', (settings.chart.yAxisTitleA[0]) ? settings.chart.yAxisTitleA[0] : '');
                _.set(highchartConfig, 'yAxis[1].title.text', (settings.chart.yAxisTitleA[1]) ? settings.chart.yAxisTitleA[1] : '');
            } else if (axisCount.length == 3) { //3 Achsen
                _.set(highchartConfig, 'yAxis[0].title.text', (settings.chart.yAxisTitleA[0]) ? settings.chart.yAxisTitleA[0] : '');
                _.set(highchartConfig, 'yAxis[1].title.text', (settings.chart.yAxisTitleA[1]) ? settings.chart.yAxisTitleA[1] : '');
                _.set(highchartConfig, 'yAxis[2].title.text', (settings.chart.yAxisTitleA[2]) ? settings.chart.yAxisTitleA[2] : '');
            } else if (axisCount.length == 4) { //4 Achse
                _.set(highchartConfig, 'yAxis[0].title.text', (settings.chart.yAxisTitleA[0]) ? settings.chart.yAxisTitleA[0] : '');
                _.set(highchartConfig, 'yAxis[1].title.text', (settings.chart.yAxisTitleA[1]) ? settings.chart.yAxisTitleA[1] : '');
                _.set(highchartConfig, 'yAxis[2].title.text', (settings.chart.yAxisTitleA[2]) ? settings.chart.yAxisTitleA[2] : '');
                _.set(highchartConfig, 'yAxis[3].title.text', (settings.chart.yAxisTitleA[3]) ? settings.chart.yAxisTitleA[3] : '');
            }
            //Plotlines to Axis
            if (!axisCount.length || axisCount.length == 1) { //1 Achse
                if (!axisCount.length) {
                    _.set(highchartConfig, 'yAxis.plotLines', settings.chart.plotLines ? settings.chart.plotLines : null);
                } else {
                    _.set(highchartConfig, 'yAxis[0].plotLines', settings.chart.plotLines ? settings.chart.plotLines : null);
                }
            } else if (axisCount.length >= 2) { //Mehr als 2 Achsen
                if (settings.chart.plotLines) {
                    ...
                    for (let i = 0; i < settings.chart.plotLines.length; i++) {
                        for (let j = 0; j < axisCount.length; j++) {
                            if (settings.chart.plotLines[i].color === axisCount[j].title.style.color) {
                                plotLineArray[j].push(settings.chart.plotLines[i]);
                            }
                        }
                    }
                    for (var x = 0; x < axisCount.length; x++) {
                        _.set(highchartConfig, 'yAxis[' + x + '].plotLines', plotLineArray[x] ? plotLineArray[x] : null);
                    }
                }
            }
        } catch (e) {
