...
class HighchartComponentController {
  ...
  constructor($element, $timeout) {
    ...
    this.$onChanges = function(changesObject) {
      if (
        changesObject.config &&
        changesObject.config.currentValue !== undefined
      ) {
        if (!initialized) {
          return;
        }
    ...
    this.removeItems = function(newItems, chartItems, id, toIgnore) {
      if (newItems && Array.isArray(newItems)) {
        ...
        for (let i = chartItems.length - 1; i >= 0; i -= 1) {
          let a = chartItems[i];
          if (
            toIgnore.indexOf(a.options.id) < 0 &&
            ids.indexOf(a.options.id) < 0
          ) {
    ...
    function initChart() {
      ...
      $timeout(
        function() {
          if (
            $element[0].clientWidth !== originalWidth ||
            $element[0].clientHeight !== originalHeight
          ) {
            ctrl.chart.reflow();
          }
        },
