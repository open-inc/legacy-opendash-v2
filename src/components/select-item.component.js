...
const logger = Logger("od-select-item");
...
class controller {
  ...
  async $onInit() {
    ...
    if (!_.isObject(this.config)) {
      throw new Error(
        "Bad usage of od-select-item config attribute. Must be an Object."
      );
    }

    if (!_.isFunction(this.watch)) {
      throw new Error(
        "Bad usage of od-select-item watch attribute. Must be a Function."
      );
    }
  ...
  searchOnChange() {
    ...
    for (const flag of this.flags) {
      this.items = this.items.filter(i =>
        flag.filter(flag.active, this.vo ? i[0] : i, this.vo ? i[1] : null)
      );
    }
  ...
  flagClass(flag) {
    return (
      flag.icon +
      (flag.active ? " od-select-item__search__flags__flag--active" : "")
    );
  }
  ...
  isSelected(e) {
    if (this.vo) {
      return _.find(this.output, o => o[0] === e[0] && o[1] === e[1])
        ? true
        : false;
    } else {
  ...
  isAvailable(e) {
    if (this.vo) {
      return _.find(this.available, a => a[0].id === e[0] && a[1] === e[1])
        ? true
        : false;
    } else {
