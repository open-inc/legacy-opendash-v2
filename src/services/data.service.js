...
export default class OpenDashDataService {
    ...
    listByType(type) {
        return _($store.toArray())
            .map(item => _.map(item.valueTypes, (v, k) => (v.type === type) ? [item, k] : null))
            .flatten()
            .pull(null)
            .value();
    }
...
class OpenDashDataContainer {
    constructor(adapter, payload) {
        ...
        if (payload.parent && !_.isString(payload.parent)) {
            throw new Error('Creating an OpenDashDataContainer with a parent requires the parent property to be a String which represents an OpenDashDataContainer/OpenDashDataItem IDs.');
        }
...
class OpenDashDataItem {
    constructor(adapter, payload) {
        ...
        if (this.validateValueTypes(payload.valueTypes)) {
            throw new Error('Creating an OpenDashDataItem requires valid value types.');
        }

        if (payload.parents && !_.isArray(payload.parents)) {
            throw new Error('Creating an OpenDashDataItem with parent(s) requires the parents property to be an array of OpenDashDataContainer/OpenDashDataItem IDs.');
        }
        ...
        this.name = payload.name;
        this.icon = payload.icon || defaultIcon;
        this.parents = _(payload.parents || []).map(x => $store.get(x)).pull(undefined).value();
        ...
        if (payload.value) {
            this.set('value', payload.value).then(null, err => logger.error('OpenDashDataItem Creation Error: Value Could Not Be Set: ', err));
        }
    }
    ...
    set(key, value, saveToDataAdapter, saveToUserAdapter) {
        if (saveToDataAdapter) {
            return $q.resolve(this.adapter.update(new OpenDashDataContext(this.adapter), {
                id: this.id,
                key,
                value,
            }));
        ...
        if (key === 'name' && value) {
            if (!_.isString(value)) {
                return $q.reject(new Error('OpenDashDataItem Update Error: Invalid name.'));
            }
        ...
        if (key === 'value' && value) {
            if (valueValidation && !this.validateValue(value, true)) {
                return $q.reject(new Error('OpenDashDataItem Update Error: Invalid value.'));
            }
        ...
        return $q.reject(new Error('[opendash/services/data] Unknown OpenDashDataItem Update Error.'));
    }
    ...
    validateValueTypes(valueTypes) {
        ...
        if (!valueTypes) {
            throw new Error('[opendash/services/data] OpenDashDataItem: Invalid property valueTypes. You need to have a valueTypes property in the payload Object when creating a new OpenDashDataItem.');
        }

        if (!_.isArray(valueTypes)) {
            throw new Error('[opendash/services/data] OpenDashDataItem: Invalid property valueTypes: Must be an array.');
        }

        if (valueTypes.length === 0) {
            throw new Error('[opendash/services/data] OpenDashDataItem: Invalid property valueTypes: Must have at least one type. If your item does not have any values, consider using an Container instead.');
        }

        valueTypes.forEach(x => {
            if (!_.isObject(x) || !x.name || !x.type) {
                throw new Error('[opendash/services/data] OpenDashDataItem: Invalid property valueTypes: Each type needs to be an object with a name and type property.');
            }

            if (types.indexOf(x.type) < 0) {
                throw new Error(`[opendash/services/data] OpenDashDataItem: Invalid property valueTypes: Unsupported type. Must be one of [${types.join(', ')}]`);
            }
        });
    }
    ...
    validateValue(value, log) {
        ...
        if (!(value.date && _.isNumber(value.date))) {
            if (log) logger.error('OpenDashDataItem: Bad Value: A value requires a valid date property, which is a Unix Millisecond Timestamp/Number.');
            return false;
        }
        ...
        if (!_.isArray(value.value) || v.length !== vt.length) {
            if (log) logger.error('OpenDashDataItem: Bad Value: A value requires a valid value array.');
            return false;
        }

        for (let i = 0; i < v.length; i++) {
            if (vt[i].type === 'String' && !_.isString(v[i])) {
                if (log) logger.error(`OpenDashDataItem: Bad Value: Value ${i} should be a String but is ${v[i]}`);
                return false;
            }

            if (vt[i].type === 'Number' && !_.isNumber(v[i])) {
                if (log) logger.error(`OpenDashDataItem: Bad Value: Value ${i} should be a Number but is ${v[i]}`);
                return false;
            }

            if (vt[i].type === 'Boolean' && !_.isBoolean(v[i])) {
                if (log) logger.error(`OpenDashDataItem: Bad Value: Value ${i} should be a Boolean but is ${v[i]}`);
                return false;
            }

            if (vt[i].type === 'Geo' && !_.isObject(v[i])) {
                if (log) logger.error(`OpenDashDataItem: Bad Value: Value ${i} should be a Geo JSON Object but is ${v[i]}`);
                return false;
            }

            if (vt[i].type === 'Object' && !_.isObject(v[i])) {
                if (log) logger.error(`OpenDashDataItem: Bad Value: Value ${i} should be a Object but is ${v[i]}`);
                return false;
            }
        }
    ...
    value() {
        ...
        return (response) ? $q.resolve(response) : $q.reject(new Error('[opendash/services/data] OpenDashDataItem.value(): Item currently has no value.'));
    }
    ...
    history(options) {
        ...
        if (options.start && options.end) {
            ...
            if (!options.start.isValid()) {
                throw new Error('[opendash/services/data] OpenDashDataItem.history(): Bad usage in options: moment(start).isValid()');
            }

            if (!options.end.isValid()) {
                throw new Error('[opendash/services/data] OpenDashDataItem.history(): Bad usage in options: moment(end).isValid()');
            }
            ...
            if (!options.since.isValid()) {
                throw new Error('[opendash/services/data] OpenDashDataItem.history(): Bad usage in options: moment(since).isValid()');
            }
            ...
            throw new Error('[opendash/services/data] OpenDashDataItem.history(): Bad usage in options.history.');
        }

        options.id = this.id;

        return $q.resolve(this.adapter.history(new OpenDashDataContext(this.adapter), options)).then(data => {
            if (!_.isArray(data)) {
                logger.error('OpenDashDataItem.history(): Bad response from Data Adapter, must be an Array.');
                return [];
            }
